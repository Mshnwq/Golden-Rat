 ...
 public:
  OneMotor(mcpwm_unit_t unit,
    ...
    mcpwm_config_t pwm_config = {
        ...
        .duty_mode = _MCPWM_DUTY_MODE_0,
        .counter_mode = _MCPWM_UP_COUNTER,
    };
  ...
  void drive(float duty) {
    ...
    if (duty > 0) {
      ...
      mcpwm_set_duty_type(unit, timer, MCPWM_OPR_A, _MCPWM_DUTY_MODE_0);
    } else {
      mcpwm_set_signal_high(unit, timer, MCPWM_OPR_A);
      mcpwm_set_duty(unit, timer, MCPWM_OPR_B, 100.0f + duty_cycle);
      mcpwm_set_duty_type(unit, timer, MCPWM_OPR_B, _MCPWM_DUTY_MODE_0);
    }
  }
