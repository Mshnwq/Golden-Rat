...
#define TIMER_CLK_PRESCALE 0    //MCPWM timer prescales
...
esp_err_t mcpwm_stop(mcpwm_unit_t mcpwm_num, mcpwm_timer_t timer_num)
{
    ...
    mcpwm_critical_enter(mcpwm_num);
    // FIXME: mcpwm_hal_timer_stop(&context[mcpwm_num].hal, timer_num);
    mcpwm_critical_exit(mcpwm_num);
    return ESP_OK;
}
                                ...
                                uint32_t red, uint32_t fed)
{
    ...
    MCPWM_TIMER_CHECK(mcpwm_num, timer_num);
    // FIXME:
    // MCPWM_CHECK(dt_mode < _MCPWM_DEADTIME_TYPE_MAX, MCPWM_DB_ERROR, ESP_ERR_INVALID_ARG);
    // mcpwm_hal_context_t *hal = &context[mcpwm_num].hal;
...
esp_err_t mcpwm_deadtime_disable(mcpwm_unit_t mcpwm_num, mcpwm_timer_t timer_num)
{
    ...
    // FIXME:
    // mcpwm_hal_context_t *hal = &context[mcpwm_num].hal;
    // mcpwm_hal_deadzone_conf_t deadzone = { .mode = _MCPWM_DEADTIME_BYPASS };
